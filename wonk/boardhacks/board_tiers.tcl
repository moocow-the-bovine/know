...
set bwtiers {grey25  grey50 grey75 grey100} ;# bw
set colortiers {"#1700cb" "#00db00" "#edf200" "#ff0000"} ;# color

#set tier_colors(bloack) $bwtiers
set tier_colors(black) $colortiers
...
set bw_ambig_rim_color "white" ;# bw
set color_ambig_rim_color "#00ffff" ;# color

#set ambig_rim_color $bw_ambig_rim_color
set ambig_rim_color $color_ambig_rim_color

proc bwtiers {} {
    global bwtiers tier_colors bw_ambig_rim_color ambig_rim_color
    set tier_colors(black) $bwtiers
    set tier_colors(white) $bwtiers
    set ambig_rim_color $bw_ambig_rim_color
    .c delete all
    ReDrawWonkBoard
}
proc colortiers {} {
    global colortiers tier_colors color_ambig_rim_color ambig_rim_color
    set tier_colors(black) $colortiers
    set tier_colors(white) $colortiers
    set ambig_rim_color $color_ambig_rim_color
    .c delete all
    ReDrawWonkBoard
}
...
;# ---- box-making
proc make_boxes {{Directory "."}} {
    global ambig_rim_color tier_colors CanvasX CanvasY
    set boxsize 100
    wm geometry . "266x318"
    .c delete all
    .c create rectangle 0 0 $boxsize $boxsize -tags "box"
    set tier_names {rim tier1 tier2 center}
    # tiers
    for {set i 0} {$i < [llength $tier_colors(black)]} {incr i} {
	set name [lindex $tier_names $i]
	.c itemconfigure "box" -fill [lindex $tier_colors(black) $i]
	.c postscript \
	    -file "${Directory}/${name}-box.ps" \
	    -x 0 -y 0 -width $boxsize -height $boxsize
	puts "Wrote file ${Directory}/${name}-box.ps"
    }
    # ambig
    .c itemconfigure "box" -fill $ambig_rim_color
    .c postscript \
	-file "${Directory}/ambig-box.ps" \
	-x 0 -y 0 -width $boxsize -height $boxsize
    puts "Wrote file ${Directory}/ambig-box.ps"
}
...
proc PlaceQCoordSpace {quadrant seg skew tier color coordList} {
	...
	if {[lsearch $ambig_rim $name] >= 0} {
	    set fillColor $ambig_rim_color
	} else {
	    set fillColor [lindex $tier_colors($color) $tier]
	}
    }
    PlaceCoordSpace $name $color $coordList $fillColor
}
;# ----- Placement: Spaces (coordinate-based)
proc PlaceCoordSpace {spaceName spaceColor coordList {fillColor {}}} {
    # marking hack
    if {$fillColor == {}} {
	set fillColor $spaceColor
    }
...
proc PlaceBoardRim {{CenterX 0} {CenterY 0} {pRadius 1}} {
    global Points tier_colors ambig_rim_color
    ...
    ;# -- place rim-fill arcs
    set angle -11.25
    set white [lindex $tier_colors(white) 0]
    set black [lindex $tier_colors(black) 0]
    set ambig $ambig_rim_color
    for {set i 0} {$i < 16} {incr i 4} {
	...
	PlaceRimChord $coords [expr $angle+45] 22.5 $ambig "chord[expr $i+2]"
